...
<%
   ...
   booklist=(List<Book>) session.getAttribute("All");
   %>
  
  欢迎买家用户:<%=session.getAttribute("username")%> &nbsp<a href="Cart">查看购物车</a>&nbsp<a href=<%="ChangePassword?username="+session.getAttribute("username")%>>修改密码</a>
  <a href="/ShopSys/index.jsp">退出登录</a> &nbsp&nbsp<form action="Search" method="post"><input type="text" name="search"/> <input type="submit" value="搜索"/></form>
   ...
   <br><h3>所有书本:</h3>
    ...
    String button="shopping.view?id="+book.getBookId()+"&seller="+book.getSeller();%>
    <tr> 
        <td><%=book.getBookId()%>
		</td>
		<td><%=book.getBookName() %> 
